...
class UIBiSage3(SequentialRecommender):

    input_type = InputType.PAIRWISE

    def __init__(self, config, dataset):
        super(UIBiSage3, self).__init__(config, dataset)
        ...
        self.user_embedding_uibi = nn.Embedding(self.n_users, self.user_embedding_size, padding_idx=0, device=self.device) # user id [1,2,...]
        self.item_embedding_uibi = nn.Embedding(self.n_items, self.item_embedding_size, padding_idx=0, device=self.device) # item id [1,2,...]
        ...
        self.user_graph = self.graph_generation(user_adj, 'user').to(self.device)
        self.item_graph = self.graph_generation(item_adj, 'item').to(self.device)
    ...
    def seq_interaction_generation(self, dataset):
        ui_matrix = torch.zeros([self.n_users, self.n_items], dtype=torch.float32)
    ...
    def forward(self, user_seq, item_seq, item_seq_len):
        self.user_graph.ndata['h0'] = self.user_embedding(torch.LongTensor(range(self.n_users)).to(self.device))
        self.item_graph.ndata['h0'] = self.item_embedding(torch.LongTensor(range(self.n_items)).to(self.device))
        self.uibi_graph.srcdata['h0'] = self.user_embedding_uibi(torch.LongTensor(range(self.n_users)).to(self.device))
        self.uibi_graph.dstdata['h0'] = self.item_embedding_uibi(torch.LongTensor(range(self.n_items)).to(self.device))
        ...
        u_uibi_emb = self.uibi_graph.ndata['h'+str(self.graph_num_way)]['user'][user_seq.unsqueeze(1).repeat((1, self.max_seq_length))]
        i_uibi_emb = self.uibi_graph.ndata['h'+str(self.graph_num_way)]['item'][item_seq]
        
        user_emb_em = torch.mul(user_emb, u_uibi_emb) # user embedding element-wise mul
        item_emb_em = torch.mul(item_emb, i_uibi_emb) # item embedding element-wise mul
        
        position_ids = torch.arange(item_seq.size(1), dtype=torch.long, device=item_seq.device)
        position_ids = position_ids.unsqueeze(0).expand_as(item_seq)
        position_embedding = self.position_embedding(position_ids)
        
        input_emb = torch.cat([user_emb_em, item_emb_em], dim=2)
    ...
    def calculate_loss(self, interaction):
        ...
        if self.loss_type == 'BPR':
            ...
            pos_inter_emb = self.uibi_graph.ndata['h'+str(self.graph_num_way)]['item'][pos_items]
            pos_items_emb = torch.mul(pos_items_emb, pos_inter_emb)
            
            neg_items_emb = self.item_graph.ndata['h'+str(self.graph_num_way)][neg_items]
            neg_inter_emb = self.uibi_graph.ndata['h'+str(self.graph_num_way)]['item'][neg_items]
            neg_items_emb = torch.mul(neg_items_emb, neg_inter_emb)
            ...
            test_uiitem_emb = self.uibi_graph.ndata['h'+str(self.graph_num_way)]['item']
            test_item_emb = torch.mul(test_item_emb, test_uiitem_emb)
    ...
    def predict(self, interaction):
        ...
        test_uiitem_emb = self.uibi_graph.ndata['h'+str(self.graph_num_way)]['item'][test_item]
        test_item_emb = torch.mul(test_item_emb, test_uiitem_emb)
