...
from recbole.model.sequential_recommender import UIBiSageMixed
from recbole.trainer import Trainer
from recbole.utils import init_seed, init_logger, InputType, set_color
import argparse

def run_recbole(model_name, data_name):
    config = Config(model=UIBiSageMixed, dataset=data_name, 
                    config_file_list=['{dataname}_mixed.yaml'.format(dataname=data_name)])
    ...
    model = UIBiSageMixed(config, train_data.dataset).to(config['device'])
    logger.info(model)
